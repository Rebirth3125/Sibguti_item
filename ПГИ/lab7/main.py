...
if __name__ == "__main__":
    ...
    if not os.path.exists(f"{path_}"):
        os.mkdir(f"{path_}")

    img = Image.open(f"{path_}//{image_name}.{image_ext}")
    ...
    with open(f"{path_}//{secret_name}" + '.' + secret_ext, 'rb') as f:
    ...
    new_img.save(f"{path_}//{image_name}_steganography.{image_ext}", "BMP")
    new_img.show()

    """Чтение закодированной информации с файла"""
    
    img = Image.open(f"{path_}//{image_name}_steganography.{image_ext}")
    img_bytes = io.BytesIO()
    img.save(img_bytes, "BMP")
    img_bytes = bytearray(img_bytes.getvalue())
