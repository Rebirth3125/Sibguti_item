...
class StartWindow(QtWidgets.QWidget, Ui_StartWindow):
    ...
    def theme_btn_click(self):
        ...
        msgBox.setText("(10) Написать программу, которая по заданному детерминированному конечному автомату построит эквивалентную регулярную грамматику (ЛЛ или ПЛ по желанию пользователя). Функцию переходов ДКА задавать в виде таблицы. Программа должна сгенерировать по построенной грамматике несколько цепочек в указанном диапазоне длин и проверить их допустимость заданным автоматом. Процессы построения цепочек и проверки их выводимости отображать на экране (по требованию). Предусмотреть возможность проверки автоматом цепочки, введённой пользователем.Алфавит, заданные начальная и конечная подцепочки и кратность вхождения некоторого символа алфавита во все цепочки языка ")
        msgBox.setStandardButtons(QtWidgets.QMessageBox.Ok)
    
        msgBox.setStyleSheet("QMessageBox {width: 800px; height: 1600px;}")
        msgBox.resize(800, 1600)
...
class MainWindow(QtWidgets.QWidget, Ui_MainWindow):
    ...
    def check_chain_btn_clicked(self):
        ...
        if(chain.find(START) != 0):
            print("начальная подцепочка не обнаружена")
            return
        for i in range(len(END)):
            if tempstr[i] != end[i]:
                print("конечная подцепочка не обнаружена")
                return

        chet = 0
        strstr = chain
        while(strstr.find(CHET[0]) != -1):
            strstr = strstr[strstr.find(CHET[0])+1:]
            chet+=1
        if ( chet % CHET[1] == 0):
            print(CHET[1],chet)
        else:
            print(f"количество символов {CHET[0]} не кратно {CHET[1]} и равно {chet}")
            return
